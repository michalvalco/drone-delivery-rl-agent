...
class DroneDeliveryEnv(gym.Env):
    ...
    def step(self, action: int):
        ...
        # Interact: pick up / deliver / charge (order of checks matters)
            ...
            # DEBUG: Log when interact is pressed
            print(f"\n🔍 DEBUG: INTERACT pressed! Drone at {self.drone_pos}, Carrying: {self.carrying_package}")
            if self.carrying_package and self.current_package:
                print(f"🔍 DEBUG: Carrying package {self.current_package.id} (priority {self.current_package.priority}) to destination {self.current_package.dest_pos}")
            ...
            if self.carrying_package and self._at_destination(self.current_package):
                print("🔍 DEBUG: About to attempt delivery!")
                delivery_reward = self._deliver_current(info)
                reward += delivery_reward
                action_performed = True
                print(f"🔍 DEBUG: Delivery complete! Reward: {delivery_reward}")
            ...
            # Pickup (must be in warehouse; easy mode: anywhere inside zone)
                ...
                print(f"🔍 DEBUG: Attempting pickup in warehouse at position {self.drone_pos}")
                self._pickup_if_available()
                if self.carrying_package:
                    reward += 10.0  # pickup bonus
                    action_performed = True
                    print(f"🔍 DEBUG: Pickup successful! Now carrying package {self.current_package.id}")
                else:
                    print("🔍 DEBUG: Pickup failed - no package available at this position")
            
            # Charge (only if didn't pickup or deliver)
            if not action_performed and self._on_charger(self.drone_pos):
                print(f"🔍 DEBUG: Attempting to charge at position {self.drone_pos}")
                if self.battery < 1.0:  # FIX: Only charge if not full
                    ...
                    action_performed = True
                    print(f"🔍 DEBUG: Charging successful! Battery: {before:.2f} → {self.battery:.2f}, Reward: {reward}")
                else:
                    print("🔍 DEBUG: Battery already full, no charging needed")
            
            # Invalid interact (nothing to interact with)
            if not action_performed:
                reward += -5.0  # useless interact
                print(f"🔍 DEBUG: Invalid interact at position {self.drone_pos} - nothing to interact with")
    ...
    def _at_destination(self, pkg: Optional[Package]) -> bool:
        if pkg is None:
            print("🔍 DEBUG _at_destination: No package to check destination for")
            return False
        
        drone_tuple = (int(self.drone_pos[0]), int(self.drone_pos[1]))
        dest_tuple = tuple(pkg.dest_pos)
        result = drone_tuple == dest_tuple
        
        print(f"🔍 DEBUG _at_destination: Drone at {drone_tuple}, Destination {dest_tuple}, Match: {result}")
        print(f"   Types: drone_pos type={type(self.drone_pos)}, dest_pos type={type(pkg.dest_pos)}")
    ...
    def _pickup_if_available(self):
        ...
        if not candidates:
            print("🔍 DEBUG _pickup_if_available: No packages available for pickup")
            return
        ...
        top = candidates[0]
        print(f"🔍 DEBUG _pickup_if_available: Top candidate package {top.id} at {top.pickup_pos}, priority {top.priority}")
        ...
        if self.easy_mode_for_tests:
            ...
            print(f"🔍 DEBUG: Easy mode pickup successful for package {top.id}")
            return
        ...
        if tuple(self.drone_pos) == tuple(top.pickup_pos):
            ...
            self.carrying_package = True
            self.current_package = top
            print(f"🔍 DEBUG: Strict mode pickup successful for package {top.id}")
        else:
            print(f"🔍 DEBUG: Strict mode pickup failed - drone at {tuple(self.drone_pos)}, package at {tuple(top.pickup_pos)}")
    ...
    def _deliver_current(self, info: Dict[str, int | float | str]) -> float:
        ...
        """
        print(f"🔍 DEBUG _deliver_current: Starting delivery attempt")
        
        if not self.carrying_package or self.current_package is None:
            print("🔍 DEBUG: Cannot deliver - not carrying or no package")
            return 0.0
        if not self._at_destination(self.current_package):
            print("🔍 DEBUG: Cannot deliver - not at destination")
            return 0.0
        
        # FIX: Calculate delivery reward (100 × priority)
        delivery_reward = 100.0 * float(self.current_package.priority)
        print(f"🔍 DEBUG: Delivery successful! Package {self.current_package.id} priority {self.current_package.priority}, reward {delivery_reward}")
        
        # Update state
        self.current_package.delivered = True
        self.current_package.picked_up = True
        self.carrying_package = False
        delivered_package = self.current_package  # Store for debug
        self.current_package = None
        info["delivered_now"] = 1
        
        print(f"🔍 DEBUG: State after delivery - carrying: {self.carrying_package}, current_package: {self.current_package}")
        print(f"🔍 DEBUG: Package {delivered_package.id} marked as delivered: {delivered_package.delivered}")
